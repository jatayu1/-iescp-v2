export default {
    template: `
    <div class="container">
        <h2 class="welcome-title">Welcome, Influencer</h2>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="table-header">Influencer Details</th>
                            <th class="table-header">Social Media Details</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <ul>
                                    <li><strong>Category:</strong> {{ influencerData.category }}</li>
                                    <li><strong>Niche:</strong> {{ influencerData.niche }}</li>
                                    <li><strong>Reach:</strong> {{ totalFollowers }} </li>
                                </ul>
                            </td>
                            <td>
                                <div class="social-links">
                                    <div class="social-link">
                                        <a :href="influencerData.social_links.instagram_link" target="_blank" class="instagram">
                                            <div class="link-content">
                                                <div class="icon-text">
                                                    <i class="fab fa-instagram"></i>
                                                    <span>Instagram</span>
                                                </div>
                                                <span>{{ influencerData.social_links.instagram_followers }} followers</span>
                                            </div>
                                        </a>
                                    </div>
                                    <div class="social-link">
                                        <a :href="influencerData.social_links.facebook_link" target="_blank" class="facebook">
                                            <div class="link-content">
                                                <div class="icon-text">
                                                    <i class="fab fa-facebook-f"></i>
                                                    <span>Facebook</span>
                                                </div>
                                                <span>{{ influencerData.social_links.facebook_followers }} followers</span>
                                            </div>
                                        </a>
                                    </div>
                                    <div class="social-link">
                                        <a :href="influencerData.social_links.youtube_link" target="_blank" class="youtube">
                                            <div class="link-content">
                                                <div class="icon-text">
                                                    <i class="fab fa-youtube"></i>
                                                    <span>YouTube</span>
                                                </div>
                                                <span>{{ influencerData.social_links.youtube_followers }} followers</span>
                                            </div>
                                        </a>
                                    </div>
                                    <div class="social-link">
                                        <a :href="influencerData.social_links.x_link" target="_blank" class="x">
                                            <div class="link-content">
                                                <div class="icon-text">
                                                    <i class="fab fa-twitter"></i>
                                                    <span>X</span>
                                                </div>
                                                <span>{{ influencerData.social_links.x_followers }} followers</span>
                                            </div>
                                        </a>
                                    </div>
                                    <div class="social-link">
                                        <a :href="influencerData.social_links.linkedin_link" target="_blank" class="linkedin">
                                            <div class="link-content">
                                                <div class="icon-text">
                                                    <i class="fab fa-linkedin-in"></i>
                                                    <span>LinkedIn</span>
                                                </div>
                                                <span>{{ influencerData.social_links.linkedin_followers }} followers</span>
                                            </div>
                                        </a>
                                    </div>
                                </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th class="table-header">Sonsored by</th>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="requests in adRequestsReceived" :key="requests.id">
                                ...
                                <td>{{ requests.flagged }}</td>
                                <td>
                                    <button class="btn btn-success btn-sm" @click="accept(requests.id, requests.sent_to)">Accept</button> | 
    ...
    methods: {
        ...
        async user_info() {
            try {
                ...
                if (response.ok) {
                    ...
                    // Fetch influencer info after user info is set
                    if (this.user.id) {
                        this.influencer_info(this.user.id);
                    }
                } else {
        ...
        async influencer_info(id) {
            try {
                const response = await fetch(`/api/influencer-details/${id}`, {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    this.influencerData.category = data.category;
                    this.influencerData.niche = data.niche;
                    this.influencerData.social_links = data.social_links;
                } else {
                    console.error('Failed to fetch user info');
                }
            } catch (error) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.campaignData = campaigns.filter(campaign => 
                        campaign.status === 1 && campaign.assigned_to === this.user.id
                    );
                }
            } catch (error) {
        ...
        async fetchAdRequests() {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(this.adRequestsReceived);
                } else {
        ...
        async rejectAdrequest(campaign_id, sent_by, sent_to) {
            const response = await fetch("/api/ad_requests", {
                method: "DELETE",
                headers: { "Content-Type": "application/json", "Authentication-Token": this.token },
                body: JSON.stringify({
        ...
        async accept(id, uid) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Re-fetch the campaign names and update the active campaigns table
                    await this.fetchCampaignNames();
                } else {
    ...
    computed: {
        totalFollowers() {
            if (!this.influencerData) return 0;
            return (this.influencerData.social_links.instagram_followers || 0) +
                   (this.influencerData.social_links.facebook_followers || 0) +
                   (this.influencerData.social_links.youtube_followers || 0) +
                   (this.influencerData.social_links.x_followers || 0) +
                   (this.influencerData.social_links.linkedin_followers || 0);
        }
    },
}
